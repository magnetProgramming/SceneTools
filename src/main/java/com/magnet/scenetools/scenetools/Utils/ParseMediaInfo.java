package com.magnet.scenetools.scenetools.Utils;

import java.io.BufferedReader;
import java.io.File;
import java.io.InputStreamReader;

public class ParseMediaInfo {
	public String parseMediaInfo(File mediaFile) {

		String mediaFilePath = mediaFile.getAbsolutePath();
		ProcessBuilder fetchMediaProcessBuild = new ProcessBuilder("ffprobe", "-v", "quiet", "-print_format", "json",
				"-show_format", "-show_streams", mediaFilePath);

		try {
			Process fetchMediaProcess = fetchMediaProcessBuild.start();

			BufferedReader reader = new BufferedReader(new InputStreamReader(fetchMediaProcess.getInputStream()));
			StringBuilder output = new StringBuilder();
			String line;

			while ((line = reader.readLine()) != null) {
				output.append(line).append("\n");
			}

			int exitCode = fetchMediaProcess.waitFor();
			if (exitCode == 0) {
				String jsonOutput = output.toString();
				return jsonOutput;
			} else {
				System.out.println("ffprobe exited with error code " + exitCode);
			}

		} catch (Exception e) {
			throw new RuntimeException("You need ffmpeg/ffprobe installed and available on PATH.", e);
		}
		return "";
	}
}
